...
std::optional<FllmModel> read_fllm(const std::string& path) {
    ...
    if (static_cast<size_t>(file_size) < sizeof(FllmHeader)) return std::nullopt;
    ...
    std::vector<uint8_t> payload(buf.begin() + sizeof(hdr), buf.end());
    uint64_t computed = sha256::hash_first8(payload.data(), payload.size());
