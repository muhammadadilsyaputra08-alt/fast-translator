...
// out[d] = sum_i W[d,i] * x[i], W is [n x d_out]? — here W is row-major
// [d_out x d_in], matching how the checkpoint stores linear layer weights.
...
// the checkpoint's fixed section ordering. `shared_classifier` mirrors the
// llama2.c convention: a positive vocab_size in the header means the token
// embedding table doubles as the output classifier (no separate wcls
// tensor stored) — true for stories15M.
void load_weights(std::ifstream& f, const Config& c, Weights& w) {
    ...
    read_vec(w.wk, static_cast<size_t>(c.n_layers) * c.dim * c.dim);
    read_vec(w.wv, static_cast<size_t>(c.n_layers) * c.dim * c.dim);
...
std::unique_ptr<Transformer> Transformer::load(const std::string& path) {
    ...
    // A negative vocab_size in the original llama2.c format signals an
    // unshared classifier; stories15M uses a positive (shared) vocab_size.
...
float* Transformer::forward(int token, int pos) {
    ...
    for (int l = 0; l < p.n_layers; ++l) {
        ...
        matmul(s.k.data(), s.xb.data(), w.wk.data() + static_cast<size_t>(l) * dim * dim, dim, dim);
        matmul(s.v.data(), s.xb.data(), w.wv.data() + static_cast<size_t>(l) * dim * dim, dim, dim);

        // RoPE: rotate each head's (q,k) pairs by position-dependent angle.
        for (int h = 0; h < p.n_heads; ++h) {
            float* q_head = s.q.data() + static_cast<size_t>(h) * head_size;
            float* k_head = s.k.data() + static_cast<size_t>(h) * head_size;
            for (int i = 0; i < head_size / 2; ++i) {
                float fcr = freq_real_row[i];
                float fci = freq_imag_row[i];
        ...
        // Write this step's K,V into the per-layer cache.
        ...
        // Multi-head causal attention over all cached positions 0..pos.
        for (int h = 0; h < p.n_heads; ++h) {
        ...
        for (int i = 0; i < hidden_dim; ++i) {
            float v = s.hb[i];
            float silu = v / (1.0f + std::exp(-v)); // SiLU(x) = x * sigmoid(x)
            s.hb[i] = silu * s.hb2[i];               // gated by w3 branch
        }
    ...
    matmul(s.logits.data(), s.x.data(), w.token_embedding_table.data(), dim, p.vocab_size);
