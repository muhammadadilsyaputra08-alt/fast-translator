...
void RunState::init(const fllm::TransformerConfig& c) {
    ...
    q.resize(c.dim); k.resize(c.dim); v.resize(c.dim);
...
float* MixedPrecisionTransformer::forward(int token, int pos) {
    ...
    for (int l = 0; l < p.n_layers; ++l) {
        ...
        matmul_int8(s.k.data(), s.xb.data(), w.wk.data() + static_cast<size_t>(l) * dim * dim, w.wk_alpha[l], dim, dim);
        matmul_int8(s.v.data(), s.xb.data(), w.wv.data() + static_cast<size_t>(l) * dim * dim, w.wv_alpha[l], dim, dim);
...
std::vector<std::vector<float>> MixedPrecisionTransformer::forward_batch(
    ...
    for (int t = 0; t < n; ++t) {
        ...
        matmul_fp32(results[t].data(), xt.data(), w.token_embedding_table.data(), dim, p.vocab_size);
    }
    return results;
}
...
float* MixedPrecisionTransformer::forward_evictable(int token, int true_pos, const kvcache::RocketKVEvictor& evictor) {
    ...
    for (int l = 0; l < p.n_layers; ++l) {
        ...
        size_t n_active_before = evict_key_cache_[l].size() / dim; // includes the just-appended entry
        for (int h = 0; h < p.n_heads; ++h) {
            const float* q_head = s.q.data() + static_cast<size_t>(h) * head_size;
            std::vector<float> att_row(n_active_before);
            for (size_t t = 0; t < n_active_before; ++t) {
                const float* k_t = evict_key_cache_[l].data() + t * dim + static_cast<size_t>(h) * head_size;
                float score = 0.0f;
                for (int i = 0; i < head_size; ++i) score += q_head[i] * k_t[i];
            ...
            std::fill(out_head, out_head + head_size, 0.0f);
            for (size_t t = 0; t < n_active_before; ++t) {
                const float* v_t = evict_value_cache_[l].data() + t * dim + static_cast<size_t>(h) * head_size;
                float a = att_row[t];
                for (int i = 0; i < head_size; ++i) out_head[i] += a * v_t[i];
            }
        }
    ...
    if (evictor.should_evict_check(active_positions_.size())) {
        ...
        for (int l = 0; l < p.n_layers; ++l) { new_keys[l].reserve(kept.size()*dim); new_values[l].reserve(kept.size()*dim); }

        for (size_t idx : kept) {
            new_positions.push_back(active_positions_[idx]);
            for (int l = 0; l < p.n_layers; ++l) {
                new_keys[l].insert(new_keys[l].end(),
                    evict_key_cache_[l].begin() + idx*dim, evict_key_cache_[l].begin() + (idx+1)*dim);
                new_values[l].insert(new_values[l].end(),
                    evict_value_cache_[l].begin() + idx*dim, evict_value_cache_[l].begin() + (idx+1)*dim);
            }
        }
